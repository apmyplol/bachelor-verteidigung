...
class Homogeneity(Slide):
    def construct(self):
        ...
        self.play(Write(VGroup(thm,thm_rect, homo_eq)), VGroup(int_subtr, intr_subtr_f, text_admod).animate.shift(2*UP))
        self.next_slide()
        ...
        cir_l = LabeledLine(MathTex("r", font_size=16), start=p0.get_corner(UL), end=cir.point_at_angle(3*PI/4), stroke_width=2)
        cir_l.add_updater(lambda lab: lab.put_start_and_end_on(start=p0.get_corner(UL), end=cir.point_at_angle(3*PI/4)))
        ...
        self.play(Write(predot), Write(m_cond))
        #self.next_slide()

        self.play(cir.animate.scale(0.8), FadeOut(VGroup(sq, m_cond)), ReplacementTransform(cir_l.label, Label(label=MathTex(r"\frac{\metro{\ve{z}}}{m}", font_size=10), box_config={"color":WHITE, "buff": 0}, frame_config={"buff":0, "stroke_width":0}).move_to(cir_l.label.get_center()+DR*0.1)))
        self.next_slide()
